...
//			分页之后的某几个设备
	...
	public List<TopoInterface> queryBy(TopoHostNode host){
		...
		try{
			Query query = session.createSQLQuery("select d.id,d.node_Id,d.if_Index,d.if_Desc,customer_Id,startTime,endTime from (select * from Device_Interface d where d.node_Id = :ipAddress) d left join  Topo_Interface t on d.node_Id = t.node_Id and d.if_index = t.if_index where t.endTime is null")
