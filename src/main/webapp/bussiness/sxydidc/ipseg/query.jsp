...
<head>
    ...
    <script type="text/javascript">
		$(function($){
			...
			window['g']=$("#maingrid").ligerGrid(queryIPSegData(parms));  			  
		});
		...
		function searchclick(parms){
    		var ipseg = $('#txtName').val();
    		if(ipseg!=null&&ipseg.trim().length!=0){
    			if(parms.length>$("#parmsLength").val()){//说明已经添加了按关键字查询条件需要删除原有的过滤条件重新添加
    				parms.pop();
    			}     		
    			parms.push({name:'ipseg.name',value:ipseg}); 
    		}else{
    			if(parms.length>$("#parmsLength").val()){//说明已经添加了按关键字查询条件需要删除原有的过滤条件重新添加
    				parms.pop();
    			}
...
<body> 
	<form id="searchform">
        <table cellpadding="0" cellspacing="0" class="l-table-edit">
            <tr>
                <td align="right" class="l-table-edit-td">IP段名称:</td>
                <td align="left" ></td>
