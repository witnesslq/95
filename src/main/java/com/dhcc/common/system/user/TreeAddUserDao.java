	...
	public boolean addUserTSLtable(String id,String cropid,String stationid,String postid,String roleid ){
		...
		try{
			...
			for(String s : strcropid){
				sql = "insert into tslusercorp values('"+id+"','"+s+"');";
				dbm.addBatch(sql);
			}
			//用户岗位关联表
	...
	public boolean updateUser(UserQueryModel model){
		...
		try {
			String sql = String.format(
					...
					model.getEmailpublic(),model.getMobilepublic(),model.getPhonepublic(),birthday, model.getArea(), model.getRemark(),model.getDcid(),
					model.getId());
			dbm.addBatch(sql);
