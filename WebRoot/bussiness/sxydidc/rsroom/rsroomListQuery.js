...
function loadInfo(){
  ...
  $.ajax({
		...
		for(var i=0;i<data.length;i++){
			 var menu = [];
		var item = data[i];
		menu.push("<input name=\"checked_info\" type=\"radio\" value='roomid:"+item.id+":"+item.roomname+"'>");   
		menu.push(item.roomcode);
		menu.push(item.roomname);
		menu.push(item.gradeValue);
	    dataSet.push(menu);
		}
		 $("#example1").DataTable({
			 "data": dataSet,//数据源
			 "columns": [
		                    ...
		                    { "title": "机房编码" ,"class":"font-center"},
		                    { "title": "机房名称" ,"class":"font-center"},
		                    { "title": "机房级别" ,"class":"font-center"}
		                ],
  ...
  window.parent.$("#RoomInfo").modal('show');
}
