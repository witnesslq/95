...
<head>
    ...
    <script type="text/javascript">
		$(function($){
			...
			window['g']=$("#maingrid").ligerGrid(queryRoomData(parms));  			  
		});
		...
		function searchclick(parms){
    		var roomname = $('#txtName').val();
    		if(roomname!=null&&roomname.trim().length!=0){
    			if(parms.length>$("#parmsLength").val()){//说明已经添加了按关键字查询条件需要删除原有的过滤条件重新添加
    				parms.pop();
    			}    		
    			parms.push({name:'rsroom.roomname',value:roomname}); 
    		}else{
    			if(parms.length>$("#parmsLength").val()){//说明已经添加了按关键字查询条件需要删除原有的过滤条件重新添加
    				parms.pop();
    			} 
...
<body> 
	<form id="searchform">
        <table cellpadding="0" cellspacing="0" class="l-table-edit">
            <tr>
                <td align="right" class="l-table-edit-td">机房名称:</td>
                <td align="left" ></td>
